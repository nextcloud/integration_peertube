OC.L10N.register(
    "integration_peertube",
    {
    "PeerTube videos" : "Βίντεο PeerTube",
    "Bad HTTP method" : "Κακή μέθοδος HTTP",
    "Connected accounts" : "Συνδεδεμένοι λογαριασμοί",
    "PeerTube integration" : "Ενσωμάτωση PeerTube",
    "Integration of PeerTube decentralized and federated video platform" : "Ενσωμάτωση της αποκεντρωμένης και ομοσπονδιακής πλατφόρμας βίντεο PeerTube",
    "PeerTube integration provides a smart picker provider to search for videos\nand a link preview widget for video links." : "Η ενσωμάτωση PeerTube παρέχει έναν έξυπνο πάροχο επιλογής για αναζήτηση βίντεο\nκαι ένα widget προεπισκόπησης συνδέσμων για συνδέσμους βίντεο.",
    "PeerTube options saved" : "Οι επιλογές PeerTube αποθηκεύτηκαν",
    "Failed to save PeerTube options" : "Αποτυχία αποθήκευσης επιλογών PeerTube",
    "PeerTube instance list (separated by commas or new lines)" : "Λίστα στιγμιότυπων PeerTube (διαχωρισμένα με κόμματα ή νέες γραμμές)",
    "Nextcloud will search and resolve video links for all those instances." : "Το Nextcloud θα αναζητά και θα επιλύει συνδέσμους βίντεο για όλα αυτά τα στιγμιότυπα.",
    "Enable searching for PeerTube videos" : "Ενεργοποίηση αναζήτησης για βίντεο PeerTube",
    "Enable PeerTube video link previews" : "Ενεργοποίηση προεπισκοπήσεων συνδέσμων βίντεο PeerTube",
    "Warning, everything you type in the search bar will be sent to some PeerTube instances." : "Προειδοποίηση, ό,τι πληκτρολογείτε στη γραμμή αναζήτησης θα αποστέλλεται σε ορισμένα στιγμιότυπα PeerTube.",
    "No video reference found" : "Δεν βρέθηκε αναφορά βίντεο"
},
"nplurals=2; plural=(n != 1);");
